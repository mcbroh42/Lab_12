...
{
   T** items = toArray();		// This returns an inorder traversal of a binary search tree and stores it to an array.
   BinarySearchTree<T>* bst = new BinarySearchTree<T>(compare_items, compare_keys);
   // instantiates a new binary tree that holds the items you're comparing and the keys
   
   //DO THIS
   // You can use the "ToArrray()" function from the last lab
   
	bst->minimize(items, 0, size-1);
...
void BinarySearchTree<T>::minimize(T** items, int first, int last)
{
   //DO THIS (recursive minimize method)
	...
	T* root = items[mid];				// midpoint of your array
...
BinarySearchTree<T>* BinarySearchTree<T>::minimizeComplete()
{
   ...
   BinarySearchTree<T>* bst = new BinarySearchTree<T>(compare_items, compare_keys);
   //DO THIS
	bst->minimizeComplete(items, 0, size-1);
